...
function LandingPage() {
  ...
  return (
    <>
      <div className={`landingContainer ${darkMode ? 'dark' : 'light'} bg-black w-[100%] h-[100vh] font-primary relative`}>
        <div className="content h-[100vh] flex flex-col justify-evenly items-center border-2 border-white-700">
          <button
              onClick={() => setDarkMode(!darkMode)}
              className={`toggleButton bg-transparent text-white px-4 py-2 border-2 border-white fixed top-0 left-0 m-4 ${darkMode ? 'border-white' : 'border-black'}`}
            >
          ...
          <div className="auth text-center">
            ...
            {!isAuthenticated ? (
              <button
                className={`bg-black text-white text-[1.5rem] sm:text-[2rem] px-[2rem] sm:px-[4rem] hover:bg-white hover:text-black border-2 border-white-700 ${darkMode ? 'border-loginwhite' : 'border-loginblack'}`}
          ...
          <div className={`inputBox h-[25%] md:h-[30%] w-[90%] sm:w-[70%] md:w-[60%] lg:w-[50%] xl:w-[35%] 2xl:w-[30%] flex flex-col items-center justify-between border-2 p-4 border-white ${darkMode ? 'border-w' : 'border-b'}`}>
            <input
              id="url"
